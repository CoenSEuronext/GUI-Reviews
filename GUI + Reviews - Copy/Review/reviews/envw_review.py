...
def run_envw_review(
    ...
    ENVW review script:
      ...
      - Apply 10% cap (iterative while-loop)
      - Shares: Target_Market_Cap / (Local_Close_Adj * FX/Index Ccy)
        where FX/Index Ccy is preferably from stock_eod_df (Index Curr == currency),
        with fallback to fx_lookup_df.
      - GBX handling: Local close in GBp is scaled to GBP by /100 before FX.
    """
    try:
        ...
        # - FX primary: from stock_eod_df where Index Curr == currency (USD)
        # - FX fallback: from fx_lookup_df using Currency (Local) -> currency
        # - Prices: from stock_eod_df / stock_co_df on #Symbol
        # ------------------------------------------------------------------
        ...
        # Primary FX from EOD slice where Index Curr == target currency (USD)
        eod_fx_slice = stock_eod_df[stock_eod_df["Index Curr"] == currency].copy()
        fx_eod_df = (
            eod_fx_slice[["#Symbol", "FX/Index Ccy"]]
            .drop_duplicates(subset=["#Symbol"], keep="first")
            .rename(columns={"FX/Index Ccy": "FX_EOD"})
        )

        # Fallback FX from lookup table
        fx_for_currency = (
            fx_lookup_df[fx_lookup_df["To_Currency"] == currency][["From_Currency", "FX/Index Ccy"]]
            .drop_duplicates(subset=["From_Currency"], keep="first")
            .rename(columns={"FX/Index Ccy": "FX_LKP"})
        )
        ...
        # Normalize Currency (Local) for FX join; keep original for GBX scaling
        ...
        base_df = (
            ...
            .merge(fx_eod_df, on="#Symbol", how="left")
            ...
            .drop(["ISIN Code:", "From_Currency", "Currency_Normalized"], axis=1)
        )

        if base_df is None or len(base_df) == 0:
            raise ValueError("Failed to load one or more required reference data files")

        # Consolidate FX: prefer FX_EOD, fallback to FX_LKP
        base_df["FX/Index Ccy"] = pd.to_numeric(base_df["FX_EOD"], errors="coerce").fillna(
            pd.to_numeric(base_df["FX_LKP"], errors="coerce")
        )
        base_df = base_df.drop(columns=["FX_EOD", "FX_LKP"], errors="ignore")

        # If local currency equals target currency, FX should be 1 (fill remaining NaNs)
        base_df.loc[
            base_df["FX/Index Ccy"].isna()
            & base_df["Currency (Local)"].astype(str).str.upper().eq(str(currency).upper()),
            "FX/Index Ccy"
        ] = 1.0

        # GBX handling: if local currency is GBX, Close Prc is typically in pence (GBp) -> convert to GBP
        base_df["Close_Prc_Local_Adj"] = pd.to_numeric(base_df["Close Prc_EOD"], errors="coerce")
        gbx_mask = base_df["Currency (Local)"].astype(str).str.upper().eq("GBX")
        base_df.loc[gbx_mask, "Close_Prc_Local_Adj"] = base_df.loc[gbx_mask, "Close_Prc_Local_Adj"] / 100.0
        ...
        missing_price_adj = int(base_df["Close_Prc_Local_Adj"].isna().sum())
        logger.info(
            f"Universe rows: {len(base_df)} | Missing #Symbol: {missing_symbol} | "
            f"Missing FX({currency}): {missing_fx} | Missing EOD price: {missing_price} | "
            f"Missing Adj price: {missing_price_adj}"
        )
        ...
        # Tie-breaker FFMC in index currency using FX and adjusted local close
        universe_df["FF_Market_Cap"] = (
            pd.to_numeric(universe_df.get("NOSH"), errors="coerce")
            * pd.to_numeric(universe_df.get("Close_Prc_Local_Adj"), errors="coerce")
        ...
        # Shares calc: MarketCap / (AdjLocalClose * FX_to_index_ccy)
        selection_df["Target_Market_Cap"] = selection_df["Weight_Final"] * index_mcap
        denom = (
            pd.to_numeric(selection_df["Close_Prc_Local_Adj"], errors="coerce")
