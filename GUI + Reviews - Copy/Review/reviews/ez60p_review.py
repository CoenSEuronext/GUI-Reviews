...
def run_ez60p_review(date, co_date, effective_date, index="EZ60P", isin="FR0013349057", 
    ...
    try:
        ...
        top_60_df = selection_df.nlargest(top_n, 'FFMC').copy()
        # Calculate the target market cap per company (equal weighting)
        target_mcap_per_company = index_mcap / top_n
        top_60_df['Unrounded NOSH'] = target_mcap_per_company / top_60_df['Close Prc_EOD']
        top_60_df['Rounded NOSH'] = top_60_df['Unrounded NOSH'].round()
        top_60_df['Free Float'] = 1
        top_60_df['Effective Date of Review'] = effective_date
        top_60_df['Currency'] = currency
        selection_df['Unrounded NOSH'] = target_mcap_per_company / selection_df['Close Prc_EOD']
        
        # Prepare EZ60P DataFrame
        EZ60P_df = (
            top_60_df[
        ...
        try:
            ...
            ez60p_path = os.path.join(output_dir, f'EZ60P_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving EZ60P output to: {ez60p_path}")
            with pd.ExcelWriter(ez60p_path) as writer:
                EZ60P_df.to_excel(writer, sheet_name='Index Composition', index=False)
