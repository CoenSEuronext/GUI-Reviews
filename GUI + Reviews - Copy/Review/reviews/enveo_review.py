...
def run_enveo_review(
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # ENVEU-style pricing plumbing:
        # - ISIN+MIC -> #Symbol mapping from stock_eod_df (RIC mode)
        # - FX comes from stock_eod_df where Index Curr == currency (EUR)
        # - EOD close comes from stock_eod_df on #Symbol
        # - CO close comes from stock_co_df on #Symbol (kept for debug; not driving pricing)
        # ------------------------------------------------------------------
        ...
        # -----------------------------
        # Map ISIN+MIC -> #Symbol (RIC mode) from EOD
        # -----------------------------
        ...
        # FX to index currency (EUR) per #Symbol, filtered on Index Curr == EUR (from EOD)
        ...
        # CO close price per #Symbol (debug / visibility)
        co_price_df = (
        ...
        # Merge symbols, FX, EOD close, CO close
        base_df = (
            base_df
        ...
        logger.info(
            ...
            f"Missing FX({currency}): {int(base_df['FX/Index Ccy'].isna().sum())} | "
            f"Missing EOD price: {int(base_df['Close_Prc_EOD'].isna().sum())} | "
        ...
        # -----------------------------
        # STEP 3: Rank ESG risk asc, tie-break FFMC desc (ENVEU-style: EOD close * FX)
        # -----------------------------
        logger.info("Step 3: Ranking by ESG Risk Score and FF Market Cap (EOD close * FX)...")

        eligible_df["NOSH_Ref"] = pd.to_numeric(eligible_df["NOSH_Ref"], errors="coerce")
        eligible_df["Close_Prc_EOD"] = pd.to_numeric(eligible_df["Close_Prc_EOD"], errors="coerce")
        ...
        # ENVEU-style price in index currency (EUR): EOD close * FX
