...
def run_eemsc_review(date, effective_date, co_date, index="EEMSC", isin="NLIX00008598", 
    ...
    Args:
        ...
        index (str, optional): Index name. Defaults to "EEMSC"
    ...
    try:
        logger.info("Starting EEMSC review calculation")
        ...
        # Flag existing companies in the EEMSC index
        # Look up ISIN codes from stock_eod_df where the Index column matches 'EEMSC'
        # Note: stock_eod_df uses 'Isin Code' (capital I, lowercase sin)
        ...
        logger.info(f"Total companies selected: {universe_df['EEMSC_selection'].sum()}")
        logger.info(f"  - Existing companies selected: {universe_df[existing_mask & (universe_df['EEMSC_selection'] == 1)].shape[0]}")
        logger.info(f"  - New companies selected: {universe_df[new_mask & (universe_df['EEMSC_selection'] == 1)].shape[0]}")
        logger.info(f"Total FFMC covered: {universe_df[universe_df['EEMSC_selection'] == 1]['Mcap in EUR'].sum() / universe_df['Mcap in EUR'].sum() * 100:.2f}%")
        ...
        selected_df = universe_df[universe_df['EEMSC_selection'] == 1].copy()
        ...
        for col in capping_columns:
            universe_df.loc[universe_df['EEMSC_selection'] == 1, col] = capped_df[col].values
        
        # For non-selected companies, set default values
        universe_df.loc[universe_df['EEMSC_selection'] == 0, 'Capping Factor'] = 1.0
        universe_df.loc[universe_df['EEMSC_selection'] == 0, 'Is Capped'] = False
        ...
        composition_df = universe_df[universe_df['EEMSC_selection'] == 1][selected_columns].copy()
        composition_df = composition_df.sort_values('Company', ascending=True)

        # Analyze inclusions and exclusions
        selection_df = universe_df[universe_df['EEMSC_selection'] == 1].copy()
        ...
        try:
            ...
            eemsc_path = os.path.join(output_dir, f'EEMSC_Review_{timestamp}.xlsx')
            
            logger.info(f"Saving output to: {eemsc_path}")
            
            # Create the Excel file with all sheets
            with pd.ExcelWriter(eemsc_path) as writer:
                # Write composition sheet
                composition_df.to_excel(writer, sheet_name='EEMSC Composition', index=False)
                ...
                inclusions_df_sorted.to_excel(writer, sheet_name='EEMSC Inclusions', index=False)

                exclusions_df_sorted = exclusions_df.sort_values('Company', ascending=True) if not exclusions_df.empty else exclusions_df
                exclusions_df_sorted.to_excel(writer, sheet_name='EEMSC Exclusions', index=False)

                # Write full universe sheet
                universe_df.to_excel(writer, sheet_name='Full Universe', index=False)
            
            # Verify file was saved
            if os.path.exists(eemsc_path):
                logger.info(f"File successfully saved to: {eemsc_path}")
                
                return {
                    "status": "success",
                    "message": "Review completed successfully",
                    "data": {
                        "eemsc_path": eemsc_path,
                        "summary": {
                            "total_companies": int(universe_df['EEMSC_selection'].sum()),
                            "existing_companies_selected": int(universe_df[existing_mask & (universe_df['EEMSC_selection'] == 1)].shape[0]),
                            "new_companies_selected": int(universe_df[new_mask & (universe_df['EEMSC_selection'] == 1)].shape[0]),
                            "total_ffmc_coverage": float(universe_df[universe_df['EEMSC_selection'] == 1]['Mcap in EUR'].sum() / universe_df['Mcap in EUR'].sum() * 100),
