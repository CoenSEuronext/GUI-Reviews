...
def run_elecp_review(date, co_date, effective_date, index="ELECP", isin="NLIX00007541", 
    ...
    try:
        ...
        elecp_universe = [
            ...
            {'Company': 'GE VERNOVA', 'ISIN': 'US36828A1016', 'MIC': 'XNYS', 'Currency': 'USD'}
        ]

        # Convert to DataFrame when needed
        elecp_df = pd.DataFrame(elecp_universe)
        ff_df = ref_data['ff']

        
        # Add the required columns to the combined dataframe
        elecp_df['Capping Factor'] = 1
        elecp_df['Effective Date of Review'] = effective_date
        ...
        top_n = 10  # 10 companies in ELECP universe
        ...
        try:
            ...
            elecp_path = os.path.join(output_dir, f'ELECP_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving ELECP output to: {elecp_path}")
            with pd.ExcelWriter(elecp_path) as writer:
                ELECP_df.to_excel(writer, sheet_name='Index Composition', index=False)
