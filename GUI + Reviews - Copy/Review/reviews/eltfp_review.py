...
def run_eltfp_review(date, co_date, effective_date, index="ELTFP", isin="NLIX00007327", 
    ...
    try:
        ...
        # ELTFP luxury/premium brands universe
        eltfp_universe = [
            ...
            {'Company': 'ESTEE LAUDER COMPANIES INC. CLASS A', 'ISIN': 'US5184391044', 'MIC': 'XNYS', 'Currency': 'USD'}
        ]

        # Convert to DataFrame when needed
        eltfp_df = pd.DataFrame(eltfp_universe)
        ff_df = ref_data['ff']

        
        # Add the required columns to the combined dataframe
        eltfp_df['Capping Factor'] = 1
        eltfp_df['Effective Date of Review'] = effective_date
        ...
        top_n = 10  # 10 companies in ELTFP universe
        ...
        try:
            ...
            eltfp_path = os.path.join(output_dir, f'ELTFP_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving ELTFP output to: {eltfp_path}")
            with pd.ExcelWriter(eltfp_path) as writer:
                ELTFP_df.to_excel(writer, sheet_name='Index Composition', index=False)
