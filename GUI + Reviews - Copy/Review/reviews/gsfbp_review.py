...
def run_gsfbp_review(date, co_date, effective_date, index="GSFBP", isin="NLIX00005735", 
    ...
    try:
        ...
        # German Equity universe
        german_equity_universe = [
            {'Company': 'ZALANDO SE', 'ISIN': 'DE000ZAL1111', 'MIC': 'XETR', 'Currency': 'EUR'},
            {'Company': 'VONOVIA SE', 'ISIN': 'DE000A1ML7J1', 'MIC': 'XETR', 'Currency': 'EUR'},
            ...
            {'Company': 'PUMA', 'ISIN': 'DE0006969603', 'MIC': 'XETR', 'Currency': 'EUR'},
            {'Company': 'KION GROUP AG', 'ISIN': 'DE000KGX8881', 'MIC': 'XETR', 'Currency': 'EUR'}
        ]

        # Convert to DataFrame when needed
        german_equity_df = pd.DataFrame(german_equity_universe)
        ff_df = ref_data['ff']

        
        # Add the required columns to the combined dataframe
        german_equity_df['Capping Factor'] = 1
        german_equity_df['Effective Date of Review'] = effective_date
        ...
        top_n = 10  # Changed from 15 to 10 for German equity universe
        ...
        try:
            ...
            gsfbp_path = os.path.join(output_dir, f'GSFBP_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving GSFBP output to: {gsfbp_path}")
            with pd.ExcelWriter(gsfbp_path) as writer:
                GSFBP_df.to_excel(writer, sheet_name='Index Composition', index=False)
