...
def run_eedf_review(date, co_date, effective_date, index="EEDF", isin="NLIX00008473", 
    ...
    try:
        ...
        eedf_universe = [
            ...
            {'Company': 'RENK GROUP', 'ISIN': 'DE000RENK730', 'MIC': 'XETR', 'Currency': 'EUR'},
            {'Company': 'HENSOLDT AG', 'ISIN': 'DE000HAG0005', 'MIC': 'XETR', 'Currency': 'EUR'},
        ...
        eedf_df = pd.DataFrame(eedf_universe)
        ff_df = ref_data['ff']

        
        # Add the required columns to the combined dataframe
        eedf_df['Capping Factor'] = 1
        eedf_df['Effective Date of Review'] = effective_date
        ...
        top_n = 10  # 15 companies in EEDF universe
        ...
        try:
            ...
            eedf_path = os.path.join(output_dir, f'EEDF_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving EEDF output to: {eedf_path}")
            with pd.ExcelWriter(eedf_path) as writer:
                EEDF_df.to_excel(writer, sheet_name='Index Composition', index=False)
