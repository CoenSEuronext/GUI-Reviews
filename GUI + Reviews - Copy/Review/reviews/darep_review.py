...
def run_darep_review(date, co_date, effective_date, index="DAREP", isin="NLIX00004837", 
                   area="US", area2="EU", type="STOCK", universe="dappt", 
    ...
    try:
        ...
        try:
            ...
            darep_path = os.path.join(output_dir, f'DAREP_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving DAREP output to: {darep_path}")
            with pd.ExcelWriter(darep_path) as writer:
                DAREP_df.to_excel(writer, sheet_name='Index Composition', index=False)
