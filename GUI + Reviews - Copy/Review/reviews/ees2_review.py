...
def run_ees2_review(date, co_date, effective_date, index="EES2", isin="NLIX00007095", 
    ...
    try:
        ...
        ees2_universe = [
            ...
            {'Company': 'CREDIT AGRICOLE', 'ISIN': 'FR0000045072', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'BNP PARIBAS ACT.A', 'ISIN': 'FR0000131104', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'SOCIETE GENERALE', 'ISIN': 'FR0000130809', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'SAFRAN', 'ISIN': 'FR0000073272', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'TOTALENERGIES', 'ISIN': 'FR0000120271', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'THALES', 'ISIN': 'FR0000121329', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'LEONARDO', 'ISIN': 'IT0003856405', 'MIC': 'MTAA', 'Currency': 'EUR'},
            {'Company': 'VALLOUREC', 'ISIN': 'FR0013506730', 'MIC': 'XPAR', 'Currency': 'EUR'}
        ]

        # Convert to DataFrame when needed
        ees2_df = pd.DataFrame(ees2_universe)
        ff_df = ref_data['ff']

        
        # Add the required columns to the combined dataframe
        ees2_df['Capping Factor'] = 1
        ees2_df['Effective Date of Review'] = effective_date
        ...
        top_n = 20  # 15 companies in EES2 universe
        ...
        try:
            ...
            ees2_path = os.path.join(output_dir, f'EES2_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving EES2 output to: {ees2_path}")
            with pd.ExcelWriter(ees2_path) as writer:
                EES2_df.to_excel(writer, sheet_name='Index Composition', index=False)
