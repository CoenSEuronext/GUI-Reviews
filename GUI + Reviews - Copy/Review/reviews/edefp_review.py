...
def run_edefp_review(date, co_date, effective_date, index="EDEFP", isin="NLIX00005982", 
    ...
    try:
        ...
        defense_aerospace_universe = [
            {'Company': 'SAAB AB B', 'ISIN': 'SE0021921269', 'MIC': 'XSTO', 'Currency': 'SEK'},
            {'Company': 'RHEINMETALL', 'ISIN': 'DE0007030009', 'MIC': 'XETR', 'Currency': 'EUR'},
            {'Company': 'MTU AERO ENGINES AG', 'ISIN': 'DE000A0D9PT0', 'MIC': 'XETR', 'Currency': 'EUR'},
            {'Company': 'BAE SYSTEMS PLC', 'ISIN': 'GB0002634946', 'MIC': 'XLON', 'Currency': 'GBX'},
            {'Company': 'ROLLS-ROYCE HOLDINGS', 'ISIN': 'GB00B63H8491', 'MIC': 'XLON', 'Currency': 'GBX'},
            {'Company': 'AIRBUS', 'ISIN': 'NL0000235190', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'DASSAULT AVIATION', 'ISIN': 'FR0014004L86', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'THALES', 'ISIN': 'FR0000121329', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'SAFRAN', 'ISIN': 'FR0000073272', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'LEONARDO', 'ISIN': 'IT0003856405', 'MIC': 'MTAA', 'Currency': 'EUR'},
        ]

        # Convert to DataFrame when needed
        defense_aerospace_df = pd.DataFrame(defense_aerospace_universe)
        ff_df = ref_data['ff']

        
        # Add the required columns to the combined dataframe
        defense_aerospace_df['Capping Factor'] = 1
        defense_aerospace_df['Effective Date of Review'] = effective_date
        ...
        selection_df = (defense_aerospace_df
           # Initial renaming
           .rename(columns={
        ...
        try:
            ...
            edefp_path = os.path.join(output_dir, f'EDEFP_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving EDEFP output to: {edefp_path}")
            with pd.ExcelWriter(edefp_path) as writer:
                EDEFP_df.to_excel(writer, sheet_name='Index Composition', index=False)
