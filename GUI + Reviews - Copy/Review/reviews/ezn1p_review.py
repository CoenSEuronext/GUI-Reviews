...
def run_ezn1p_review(date, co_date, effective_date, index="EZN1P", isin="NL0012949143", 
    ...
    try:
        ...
        top_150_eligible = eligible_df.nlargest(top_n, 'FFMC').copy()
        companies_51_150 = top_150_eligible.iloc[50:150].copy()
        # Create top_51_150_df by merging back with full selection_df to preserve all columns including exclusion info
        top_51_150_df = selection_df[selection_df['ISIN code'].isin(companies_51_150['ISIN code'])].copy()
        
        # Calculate the target market cap per company (equal weighting)
        target_mcap_per_company = index_mcap / top_51_150
        top_51_150_df['Unrounded NOSH'] = target_mcap_per_company / top_51_150_df['Close Prc_EOD']
        top_51_150_df['Rounded NOSH'] = top_51_150_df['Unrounded NOSH'].round()
        top_51_150_df['Free Float'] = 1
        top_51_150_df['Effective Date of Review'] = effective_date
        top_51_150_df['Currency'] = currency
        selection_df['Unrounded NOSH'] = target_mcap_per_company / selection_df['Close Prc_EOD']
        
        # Prepare EZN1P DataFrame
        EZN1P_df = (
            top_51_150_df[
        ...
        try:
            ...
            ezn1p_path = os.path.join(output_dir, f'EZN1P_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving EZN1P output to: {ezn1p_path}")
            with pd.ExcelWriter(ezn1p_path) as writer:
                EZN1P_df.to_excel(writer, sheet_name='Index Composition', index=False)
