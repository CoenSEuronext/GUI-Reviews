...
def get_mnemo_from_filename(filename):
    ...
    mnemos = ["CANP", "CANPT", "DAPPR", "DAPPT", "DASP", "DASPT", "DEUP", "DEUPT", 
              ...
              "AEXEW", "BNEW", "CACEW", "CLEW", "FRD4P", "FRI4P"]
    ...
    pattern = r'(CANP|CANPT|DAPPR|DAPPT|DASP|DASPT|DEUP|DEUPT|DEZP|DEZPT|DNAP|DNAPT|DPAP|DPAPT|ECHP|ECHPT|EDWP|EDWPT|EJPP|EJPPT|EUKP|EUKPT|EUSP|EUSPT|AEXEW|BNEW|CACEW|CLEW|FRD4P|FRI4P)[_-]'
...
def compare_files(coen_file, dataiku_file):
    ...
    coen_df = load_excel_data(coen_file)
    dataiku_df = load_excel_data(dataiku_file)
    ...
    base_fields = ['MIC', 'Number of Shares', 'Free Float', 'Effective Date of Review', 'Currency']
...
def create_excel_report(results, output_path):
    ...
    for mnemo, result in results.items():
        if result['status'] == 'SUCCESS':
            ...
            row_data = {
                ...
                'Date Match': field_statuses.get('Effective Date of Review', False),
    ...
    match_columns = ['MIC Match', 'Shares Match', 'Free Float Match', 'Capping Match', 'Date Match', 'Currency Match', 
