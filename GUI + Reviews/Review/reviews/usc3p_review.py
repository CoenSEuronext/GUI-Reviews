...
def run_usc3p_review(date, co_date, effective_date, index="USC3P", isin="FR0014005GE2", 
    ...
    """
    Run the USC3P index review calculation
    ...
    try:
        ...
        USC3P_df = USC3P_df.rename(columns={
            'ISIN': 'ISIN Code',
            'Rounded NOSH': 'Number of Shares',
            'Currency (Local)': 'Currency'
        })
        USC3P_df = USC3P_df.sort_values('Company')
        ...
        try:
            ...
            usc3p_path = os.path.join(output_dir, f'USC3P_df_{timestamp}.xlsx')
            
            # Save with timestamp to avoid any conflicts
            logger.info(f"Saving USC3P output to: {usc3p_path}")
            with pd.ExcelWriter(usc3p_path) as writer:
                # Write each DataFrame to a different sheet
                USC3P_df.to_excel(writer, sheet_name='Index Composition', index=False)
