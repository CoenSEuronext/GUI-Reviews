...
def run_uscle_review(date, co_date, effective_date, index="USCLE", isin="FR0014005IJ7", 
    ...
    """
    Run the USCLE index review calculation
    ...
    Args:
        ...
        index (str, optional): Index name. Defaults to "USCLE"
    ...
    try:
        ...
        USCLE_df = USCLE_df.rename(columns={
            'ISIN': 'ISIN Code',
            'Rounded NOSH': 'Number of Shares',
            'Currency (Local)': 'Currency'
        })
        USCLE_df = USCLE_df.sort_values('Company')
        ...
        try:
            ...
            uscle_path = os.path.join(output_dir, f'USCLE_df_{timestamp}.xlsx')
            
            # Save with timestamp to avoid any conflicts
            logger.info(f"Saving USCLE output to: {uscle_path}")
            with pd.ExcelWriter(uscle_path) as writer:
                # Write each DataFrame to a different sheet
                USCLE_df.to_excel(writer, sheet_name='Index Composition', index=False)
