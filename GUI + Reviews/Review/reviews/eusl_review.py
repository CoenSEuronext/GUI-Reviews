...
def run_eusl_review(date, effective_date, co_date, index="EUSL", isin="NLIX00008655", 
    ...
    Args:
        ...
        index (str, optional): Index name. Defaults to "EUSL"
    ...
    try:
        logger.info("Starting EUSL review calculation")
        ...
        # Flag existing companies in the EUSL index
        # Look up ISIN codes from stock_eod_df where the Index column matches 'EUSL'
        # Note: stock_eod_df uses 'Isin Code' (capital I, lowercase sin)
        ...
        universe_df['EUSL_selection'] = 0
        
        existing_mask = universe_df['Is_Existing'] == 1
        new_mask = universe_df['Is_Existing'] == 0
        
        universe_df.loc[existing_mask & (universe_df['Country_Cumulative_Percentage'] >= 83), 'EUSL_selection'] = 1
        universe_df.loc[new_mask & (universe_df['Country_Cumulative_Percentage'] >= 87), 'EUSL_selection'] = 1
        
        # Print summary statistics per country
        logger.info(f"\nEUSL Selection Summary by Country:")
        for country in universe_df['Country'].dropna().unique():
            country_df = universe_df[universe_df['Country'] == country]
            selected_count = country_df['EUSL_selection'].sum()
            existing_selected = country_df[existing_mask & (country_df['EUSL_selection'] == 1)].shape[0]
            new_selected = country_df[new_mask & (country_df['EUSL_selection'] == 1)].shape[0]
            logger.info(f"{country}: {selected_count} total ({existing_selected} existing, {new_selected} new)")
        
        # Print overall summary statistics
        logger.info(f"\nOverall EUSL Selection Summary:")
        logger.info(f"Total companies selected: {universe_df['EUSL_selection'].sum()}")
        logger.info(f"  - Existing companies selected: {universe_df[existing_mask & (universe_df['EUSL_selection'] == 1)].shape[0]}")
        logger.info(f"  - New companies selected: {universe_df[new_mask & (universe_df['EUSL_selection'] == 1)].shape[0]}")
        logger.info(f"Total FFMC covered: {universe_df[universe_df['EUSL_selection'] == 1]['Mcap in EUR'].sum() / universe_df['Mcap in EUR'].sum() * 100:.2f}%")
        ...
        selected_df = universe_df[universe_df['EUSL_selection'] == 1].copy()
        ...
        for col in capping_columns:
            universe_df.loc[universe_df['EUSL_selection'] == 1, col] = capped_df[col].values
        
        # For non-selected companies, set default values
        universe_df.loc[universe_df['EUSL_selection'] == 0, 'Capping Factor'] = 1.0
        universe_df.loc[universe_df['EUSL_selection'] == 0, 'Is Capped'] = False
        ...
        composition_df = universe_df[universe_df['EUSL_selection'] == 1][selected_columns].copy()
        composition_df = composition_df.sort_values('Company', ascending=True)

        # Analyze inclusions and exclusions
        selection_df = universe_df[universe_df['EUSL_selection'] == 1].copy()
        ...
        try:
            ...
            eusl_path = os.path.join(output_dir, f'EUSL_Review_{timestamp}.xlsx')
            
            logger.info(f"Saving output to: {eusl_path}")
            
            # Create the Excel file with all sheets
            with pd.ExcelWriter(eusl_path) as writer:
                # Write composition sheet
                composition_df.to_excel(writer, sheet_name='EUSL Composition', index=False)
                ...
                inclusions_df_sorted.to_excel(writer, sheet_name='EUSL Inclusions', index=False)

                exclusions_df_sorted = exclusions_df.sort_values('Company', ascending=True) if not exclusions_df.empty else exclusions_df
                exclusions_df_sorted.to_excel(writer, sheet_name='EUSL Exclusions', index=False)

                # Write full universe sheet
                universe_df.to_excel(writer, sheet_name='Full Universe', index=False)
            
            # Verify file was saved
            if os.path.exists(eusl_path):
                logger.info(f"File successfully saved to: {eusl_path}")
                ...
                return {
                    ...
                    "data": {
                        "eusl_path": eusl_path,
                        "summary": {
                            "total_companies": int(universe_df['EUSL_selection'].sum()),
                            "existing_companies_selected": int(universe_df[existing_mask & (universe_df['EUSL_selection'] == 1)].shape[0]),
                            "new_companies_selected": int(universe_df[new_mask & (universe_df['EUSL_selection'] == 1)].shape[0]),
                            "total_ffmc_coverage": float(universe_df[universe_df['EUSL_selection'] == 1]['Mcap in EUR'].sum() / universe_df['Mcap in EUR'].sum() * 100),
