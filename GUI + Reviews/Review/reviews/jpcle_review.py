...
def run_jpcle_review(date, co_date, effective_date, index="JPCLE", isin="FRESG0000215", 
                    area="EU", area2="US", type="STOCK", universe="asia_pacific_500", 
                    feed="Reuters", currency="EUR", year=None):
    """
    Run the JPCLE index review calculation
    ...
    Args:
        ...
        index (str, optional): Index name. Defaults to "JPCLE"
    ...
    try:
        ...
        ref_data = load_reference_data(current_data_folder, ['ff', 'asia_pacific_500', 'oekom_trustcarbon'])
        
        # Extract the needed DataFrames
        ff_df = ref_data['ff']
        asia_pacific_500_df = ref_data['asia_pacific_500']
        ...
        # Add Free Float data
        asia_pacific_500_df = asia_pacific_500_df.merge(
        ...
        # NOW create numeric helper columns in asia_pacific_500_df AFTER merge
        asia_pacific_500_df['Trust Metric Numeric'] = pd.to_numeric(
            asia_pacific_500_df['Reported Emissions - Emissions Trust Metric'], 
            errors='coerce'
        )
        
        asia_pacific_500_df['Coal Mining Numeric'] = pd.to_numeric(
            asia_pacific_500_df['Thermal Coal Mining - Maximum Percentage of Revenues (%)'], 
            errors='coerce'
        )
        
        asia_pacific_500_df['Thermal Power Numeric'] = pd.to_numeric(
            asia_pacific_500_df['Power Generation - Thermal Maximum Percentage of Revenues (%)'], 
            errors='coerce'
        )
        ...
        asia_pacific_500_df['Carbon Budget Rank'] = temp_rank.rank(
            method='min',
            ascending=True
        )
        ...
        asia_pacific_500_df[f'exclusion_{exclusion_count}_TrustMetric'] = None

        # Get ISINs with Trust Metric < 0.6 or 'Not Collected'
        excluded_trust_metric = asia_pacific_500_df[
            (asia_pacific_500_df['Trust Metric Numeric'] < 0.6) |
            (asia_pacific_500_df['Reported Emissions - Emissions Trust Metric'] == 'Not Collected')
        ]['ISIN'].tolist()

        asia_pacific_500_df[f'exclusion_{exclusion_count}_TrustMetric'] = np.where(
            asia_pacific_500_df['ISIN'].isin(excluded_trust_metric),
            'exclude_TrustMetric',
            None
        )
        ...
        asia_pacific_500_df[f'exclusion_{exclusion_count}_NBROverallFlag'] = None
        NBR_Overall_Flag_Red = asia_pacific_500_df[
            (asia_pacific_500_df['NBR Overall Flag'] == 'RED') |
            (asia_pacific_500_df['NBR Overall Flag'] == 'Not Collected')
        ]['ISIN'].tolist()

        asia_pacific_500_df[f'exclusion_{exclusion_count}_NBROverallFlag'] = np.where(
            asia_pacific_500_df['ISIN'].isin(NBR_Overall_Flag_Red),
        ...
        for column, (exclude_value, label) in weapons_columns.items():
            asia_pacific_500_df[f'exclusion_{exclusion_count}_{label}'] = None
            
            # Get ISINs for this weapon type (Red or 'Not Collected')
            flagged_isins = asia_pacific_500_df[
                (asia_pacific_500_df[column] == 'RED') |
                (asia_pacific_500_df[column] == 'Not Collected')
            ]['ISIN'].tolist()
            
            asia_pacific_500_df[f'exclusion_{exclusion_count}_{label}'] = np.where(
                asia_pacific_500_df['ISIN'].isin(flagged_isins),
                exclude_value,
                None
            )
        ...
        asia_pacific_500_df[f'exclusion_{exclusion_count}_CoalMining'] = None

        excluded_coal_mining = asia_pacific_500_df[
            (asia_pacific_500_df['Coal Mining Numeric'] > 0) |
            (asia_pacific_500_df['Thermal Coal Mining - Maximum Percentage of Revenues (%)'] == 'Not Collected') |
            (asia_pacific_500_df['Thermal Coal Mining - Maximum Percentage of Revenues (%)'] == 'Not Disclosed')
        ]['ISIN'].tolist()

        asia_pacific_500_df[f'exclusion_{exclusion_count}_CoalMining'] = np.where(
            asia_pacific_500_df['ISIN'].isin(excluded_coal_mining),
            'exclude_CoalMining',
            None
        )
        ...
        asia_pacific_500_df[f'exclusion_{exclusion_count}_ThermalPower'] = None

        excluded_thermal_power = asia_pacific_500_df[
            (asia_pacific_500_df['Thermal Power Numeric'] > 0.1) |
            (asia_pacific_500_df['Power Generation - Thermal Maximum Percentage of Revenues (%)'] == 'Not Collected') |
            (asia_pacific_500_df['Power Generation - Thermal Maximum Percentage of Revenues (%)'] == 'Not Disclosed')
        ]['ISIN'].tolist()

        asia_pacific_500_df[f'exclusion_{exclusion_count}_ThermalPower'] = np.where(
            asia_pacific_500_df['ISIN'].isin(excluded_thermal_power),
        ...
        excluded_shale = asia_pacific_500_df[
            (asia_pacific_500_df['Shale Oil and/or Gas - Involvement tie'] == 'Production') |
            (asia_pacific_500_df['Shale Oil and/or Gas - Involvement tie'] == 'Not Collected') |
            (asia_pacific_500_df['Shale Oil and/or Gas - Involvement tie'] == 'Not Disclosed')
        ]['ISIN'].tolist()

        asia_pacific_500_df[f'exclusion_{exclusion_count}_ShaleOilGas'] = np.where(
            asia_pacific_500_df['ISIN'].isin(excluded_shale),
            'exclude_ShaleOilGas',
            None
        )
        ...
        asia_pacific_500_df[f'exclusion_{exclusion_count}_CarbonBudget'] = np.where(
            asia_pacific_500_df['Carbon Budget Rank'] > cutoff_rank,
            'exclude_CarbonBudget',
            None
        )

        excluded_count = asia_pacific_500_df[f'exclusion_{exclusion_count}_CarbonBudget'].notna().sum()
        ...
        exclusion_columns = [col for col in asia_pacific_500_df.columns if col.startswith('exclusion_')]
        ...
        asia_pacific_500_df['Exclusion Summary'] = asia_pacific_500_df.apply(
            lambda row: summarize_exclusions(row, exclusion_columns), axis=1
        )

        asia_pacific_500_df['Excluded'] = asia_pacific_500_df['Exclusion Summary'].apply(
        ...
        JPCLE_df = JPCLE_df.rename(columns={
            'ISIN': 'ISIN Code',
            'Rounded NOSH': 'Number of Shares',
            'Currency (Local)': 'Currency'
        })
        JPCLE_df = JPCLE_df.sort_values('Company')
        ...
        try:
            ...
            jpcle_path = os.path.join(output_dir, f'JPCLE_df_{timestamp}.xlsx')
            
            # Save with timestamp to avoid any conflicts
            logger.info(f"Saving JPCLE output to: {jpcle_path}")
            with pd.ExcelWriter(jpcle_path) as writer:
                # Write each DataFrame to a different sheet
                JPCLE_df.to_excel(writer, sheet_name='Index Composition', index=False)
