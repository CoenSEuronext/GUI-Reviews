...
def run_derep_review(date, co_date, effective_date, index="DEREP", isin="NLIX00004860", 
    ...
    try:
        ...
        try:
            ...
            derep_path = os.path.join(output_dir, f'DEREP_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving DEREP output to: {derep_path}")
            with pd.ExcelWriter(derep_path) as writer:
                DEREP_df.to_excel(writer, sheet_name='Index Composition', index=False)
