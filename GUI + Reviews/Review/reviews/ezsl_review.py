...
def run_ezsl_review(date, effective_date, co_date, index="EZSL", isin="NLIX00008655", 
    ...
    Args:
        ...
        index (str, optional): Index name. Defaults to "EZSL"
    ...
    try:
        logger.info("Starting EZSL review calculation")
        ...
        # Flag existing companies in the EZSL index
        # Look up ISIN codes from stock_eod_df where the Index column matches 'EZSL'
        # Note: stock_eod_df uses 'Isin Code' (capital I, lowercase sin)
        ...
        logger.info(f"Total companies selected: {universe_df['EZSL_selection'].sum()}")
        logger.info(f"  - Existing companies selected: {universe_df[existing_mask & (universe_df['EZSL_selection'] == 1)].shape[0]}")
        logger.info(f"  - New companies selected: {universe_df[new_mask & (universe_df['EZSL_selection'] == 1)].shape[0]}")
        logger.info(f"Total FFMC covered: {universe_df[universe_df['EZSL_selection'] == 1]['Mcap in EUR'].sum() / universe_df['Mcap in EUR'].sum() * 100:.2f}%")
        ...
        selected_df = universe_df[universe_df['EZSL_selection'] == 1].copy()
        ...
        for col in capping_columns:
            universe_df.loc[universe_df['EZSL_selection'] == 1, col] = capped_df[col].values
        
        # For non-selected companies, set default values
        universe_df.loc[universe_df['EZSL_selection'] == 0, 'Capping Factor'] = 1.0
        universe_df.loc[universe_df['EZSL_selection'] == 0, 'Is Capped'] = False
        ...
        composition_df = universe_df[universe_df['EZSL_selection'] == 1][selected_columns].copy()
        composition_df = composition_df.sort_values('Company', ascending=True)

        # Analyze inclusions and exclusions
        selection_df = universe_df[universe_df['EZSL_selection'] == 1].copy()
        ...
        try:
            ...
            ezsl_path = os.path.join(output_dir, f'EZSL_Review_{timestamp}.xlsx')
            
            logger.info(f"Saving output to: {ezsl_path}")
            
            # Create the Excel file with all sheets
            with pd.ExcelWriter(ezsl_path) as writer:
                # Write composition sheet
                composition_df.to_excel(writer, sheet_name='EZSL Composition', index=False)
                ...
                inclusions_df_sorted.to_excel(writer, sheet_name='EZSL Inclusions', index=False)

                exclusions_df_sorted = exclusions_df.sort_values('Company', ascending=True) if not exclusions_df.empty else exclusions_df
                exclusions_df_sorted.to_excel(writer, sheet_name='EZSL Exclusions', index=False)

                # Write full universe sheet
                universe_df.to_excel(writer, sheet_name='Full Universe', index=False)
            
            # Verify file was saved
            if os.path.exists(ezsl_path):
                logger.info(f"File successfully saved to: {ezsl_path}")
                ...
                return {
                    ...
                    "data": {
                        "ezsl_path": ezsl_path,
                        "summary": {
                            "total_companies": int(universe_df['EZSL_selection'].sum()),
                            "existing_companies_selected": int(universe_df[existing_mask & (universe_df['EZSL_selection'] == 1)].shape[0]),
                            "new_companies_selected": int(universe_df[new_mask & (universe_df['EZSL_selection'] == 1)].shape[0]),
                            "total_ffmc_coverage": float(universe_df[universe_df['EZSL_selection'] == 1]['Mcap in EUR'].sum() / universe_df['Mcap in EUR'].sum() * 100),
