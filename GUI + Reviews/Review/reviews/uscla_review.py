...
def run_uscla_review(date, co_date, effective_date, index="USCLA", isin="FR0014005GE2", 
                    area="EU", area2="US", type="STOCK", universe="north_america_500", 
                    feed="Reuters", currency="USD", year=None):
    """
    Run the USCLA index review calculation
    ...
    Args:
        ...
        index (str, optional): Index name. Defaults to "USCLA"
    ...
    try:
        ...
        final_selection_df = top_250_df.head(25).copy()
        ...
        USCLA_df = USCLA_df.rename(columns={
            'ISIN': 'ISIN Code',
            'Rounded NOSH': 'Number of Shares',
            'Currency (Local)': 'Currency'
        })
        USCLA_df = USCLA_df.sort_values('Company')
        ...
        try:
            ...
            uscla_path = os.path.join(output_dir, f'USCLA_df_{timestamp}.xlsx')
            
            # Save with timestamp to avoid any conflicts
            logger.info(f"Saving USCLA output to: {uscla_path}")
            with pd.ExcelWriter(uscla_path) as writer:
                # Write each DataFrame to a different sheet
                USCLA_df.to_excel(writer, sheet_name='Index Composition', index=False)
