...
def run_ez40p_review(date, co_date, effective_date, index="EZ40P", isin="FR0013349057", 
    ...
    try:
        ...
        try:
            ...
            ez40p_path = os.path.join(output_dir, f'EZ40P_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving EZ40P output to: {ez40p_path}")
            with pd.ExcelWriter(ez40p_path) as writer:
                EZ40P_df.to_excel(writer, sheet_name='Index Composition', index=False)
