...
def run_tcamp_review(date, co_date, effective_date, index="TCAMP", isin="FRESG0000348", 
    ...
    try:
        ...
        # Create selection_df from stock_eod_df for TCAMP constituents
        logger.info("Creating selection_df from TCAMP constituents...")
        selection_df = stock_eod_df[stock_eod_df['Index'] == 'TCAMP'].copy()

        if selection_df.empty:
            raise ValueError("No constituents found with Index = 'TCAMP' in stock_eod_df")
        ...
        logger.info(f"Found {len(selection_df)} TCAMP constituents")
        ...
        if selection_df['Target Weight'].isna().any():
            ...
            raise ValueError(f"Invalid Mnemo values found: {invalid_mnemos}. Expected 'USCEG' or 'EZCLG'")
        ...
        try:
            ...
            tcamp_path = os.path.join(output_dir, f'TCAMP_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving TCAMP output to: {tcamp_path}")
            with pd.ExcelWriter(tcamp_path) as writer:
                TCAMP_df.to_excel(writer, sheet_name='Index Composition', index=False)
