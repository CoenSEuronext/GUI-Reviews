...
def run_clew_review(date, co_date, effective_date, index="CLEW", isin="FR0012663292", 
    ...
    try:
        ...
        selection_df['Unrounded NOSH'] = (index_mcap / 60)/ selection_df['Close Prc_EOD']
        ...
        try:
            ...
            clew_path = os.path.join(output_dir, f'CLEW_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving CLEW output to: {clew_path}")
            with pd.ExcelWriter(clew_path) as writer:
                CLEW_df.to_excel(writer, sheet_name='Index Composition', index=False)
