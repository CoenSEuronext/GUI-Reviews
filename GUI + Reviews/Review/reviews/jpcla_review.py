...
def run_jpcla_review(date, co_date, effective_date, index="JPCLA", isin="FRESG0000181", 
    ...
    Args:
        ...
        index (str, optional): Index name. Defaults to "JPCLA"
    ...
    try:
        ...
        # Calculate the target market cap per company (equal weighting across all 30 companies)
        top_n = 30
        ...
        JPCLA_df = JPCLA_df.rename(columns={
            'ISIN': 'ISIN Code',
            'Rounded NOSH': 'Number of Shares',
            'Currency (Local)': 'Currency'
        })
        JPCLA_df = JPCLA_df.sort_values('Company')
        ...
        try:
            ...
            jpcla_path = os.path.join(output_dir, f'JPCLA_df_{timestamp}.xlsx')
            
            # Save with timestamp to avoid any conflicts
            logger.info(f"Saving JPCLA output to: {jpcla_path}")
            with pd.ExcelWriter(jpcla_path) as writer:
                # Write each DataFrame to a different sheet
                JPCLA_df.to_excel(writer, sheet_name='Index Composition', index=False)
