...
def run_fr20p_review(date, co_date, effective_date, index="FR20P", isin="FR0013349057", 
    ...
    try:
        ...
        # Sort by FFMC and select the top 20 companies with highest FFMC values 
        top_n = 20
        logger.info(f"Selecting top {top_n} companies with highest FFMC...")
        top_20_df = selection_df.nlargest(top_n, 'FFMC').copy()
        # Calculate the target market cap per company (equal weighting)
        target_mcap_per_company = index_mcap / top_n
        top_20_df['Unrounded NOSH'] = target_mcap_per_company / top_20_df['Close Prc_EOD']
        top_20_df['Rounded NOSH'] = top_20_df['Unrounded NOSH'].round()
        top_20_df['Capping Factor'] = 1
        top_20_df['Free Float'] = 1
        top_20_df['Effective Date of Review'] = effective_date
        top_20_df['Currency'] = currency
        selection_df['Unrounded NOSH'] = target_mcap_per_company / selection_df['Close Prc_EOD']
        # Prepare FR20P DataFrame
        FR20P_df = (
            top_20_df[
        ...
        try:
            ...
            fr20p_path = os.path.join(output_dir, f'FR20P_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving FR20P output to: {fr20p_path}")
            with pd.ExcelWriter(fr20p_path) as writer:
                FR20P_df.to_excel(writer, sheet_name='Index Composition', index=False)
