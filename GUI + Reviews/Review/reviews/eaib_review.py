...
def run_eaib_review(date, co_date, effective_date, index="EAIB", isin="NLIX00008366", 
    ...
    try:
        ...
        eaib_universe = [
            ...
            {'Company': 'ADYEN', 'ISIN': 'NL0012969182', 'MIC': 'XAMS', 'Currency': 'EUR'},
            ...
            {'Company': 'CREDO TECHNOLOGY', 'ISIN': 'KYG254571055', 'MIC': 'XNGS', 'Currency': 'USD'},
        ...
        eaib_df = pd.DataFrame(eaib_universe)
        ff_df = ref_data['ff']

        
        # Add the required columns to the combined dataframe
        eaib_df['Capping Factor'] = 1
        eaib_df['Effective Date of Review'] = effective_date
        ...
        top_n = 15  # 15 companies in EAIB universe
        ...
        try:
            ...
            eaib_path = os.path.join(output_dir, f'EAIB_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving EAIB output to: {eaib_path}")
            with pd.ExcelWriter(eaib_path) as writer:
                EAIB_df.to_excel(writer, sheet_name='Index Composition', index=False)
