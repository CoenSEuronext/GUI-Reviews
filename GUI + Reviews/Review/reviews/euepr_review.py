...
# ---------------------------------------------------------------------------
# US MIC codes - only companies listed on these exchanges are in the universe
# ---------------------------------------------------------------------------
US_MIC_CODES = ['XNYS', 'XNGS', 'BATS', 'XNMS', 'XNCM', 'XASE', 'XNAS']
...
# Controversial Weapons - category of involvement (string or 0)
...
# Unconventional Oil & Gas - category of involvement (string or 0)
...
def run_euepr_review(date, co_date, effective_date, index="EUEPR", isin="NL0015000A61",
                     area="US", area2="EU", type="STOCK", universe="north_america_500",
                     feed="Reuters", currency="USD", year=None):
    """
    Euronext US ESGL 20 EW Index Review

    Methodology:
    - Universe: Euronext North America 500 index, filtered to US MIC codes only.
    - Step 2a: Apply Sustainalytics activity and controversy exclusions.
    - Step 2b: Rank remaining companies by FFMC (Close Prc_CO). Top 30 are eligible.
    - Step 3:  Rank the 30 eligible companies by Sustainalytics ESG Risk Score
               ascending. Tie-breaker: highest FFMC (Close Prc_CO).
    - Step 4:  Top 20 by ESG Risk Score ranking are selected.
    ...
    try:
        ...
        ref_data = load_reference_data(
            current_data_folder,
            ['ff', 'sustainalytics', 'north_america_500']
        )

        if ref_data.get('north_america_500') is None:
            raise ValueError("Failed to load north_america_500 universe data")

        # Step 1: Index Universe - North America 500, filtered to US MIC codes only
        na500_universe = ref_data['north_america_500']
        ff_df = ref_data['ff']

        universe_df = pd.DataFrame(na500_universe)
        logger.info(f"North America 500 universe size: {len(universe_df)}")

        # Filter to US companies only
        universe_df = universe_df[universe_df['MIC'].isin(US_MIC_CODES)].copy()
        logger.info(f"Universe size after US MIC filter: {len(universe_df)}")
        ...
        if sustainalytics_raw is None:
            raise ValueError("Sustainalytics data required for EUEPR index")
        ...
        exclusion_flags['excl_controversy'] = to_numeric(selection_df, CONTROVERSY_LEVEL_CODE) >= 4

        # -- Tobacco Production: Level ID > 0 (derived turnover > 0%)
        exclusion_flags['excl_tobacco_production'] = to_numeric(selection_df, TOBACCO_PRODUCTION_CODE) > 0

        # -- Tobacco Retail: Level ID >= 2 (derived turnover >= 5%)
        exclusion_flags['excl_tobacco_retail'] = to_numeric(selection_df, TOBACCO_RETAIL_CODE) >= 2

        # -- Military Contracting Weapons: Level ID > 0 (derived turnover > 0%)
        exclusion_flags['excl_military_weapons'] = to_numeric(selection_df, MILITARY_WEAPONS_CODE) > 0

        # -- Military Contracting Weapon-related: Level ID >= 3 (derived turnover >= 10%)
        exclusion_flags['excl_military_related'] = to_numeric(selection_df, MILITARY_RELATED_CODE) >= 3
        ...
        exclusion_flags['excl_small_arms_assault'] = to_numeric(selection_df, SMALL_ARMS_ASSAULT_CODE) > 0

        # -- Small Arms Civilian Non-assault: Level ID > 0 (derived turnover > 0%)
        exclusion_flags['excl_small_arms_non_assault'] = to_numeric(selection_df, SMALL_ARMS_NON_ASSAULT_CODE) > 0
        ...
        exclusion_flags['excl_thermal_coal_extraction'] = to_numeric(selection_df, THERMAL_COAL_EXTRACTION_CODE) > 0

        # -- Thermal Coal Power Generation: Level ID > 0 (derived turnover > 0%)
        exclusion_flags['excl_thermal_coal_power'] = to_numeric(selection_df, THERMAL_COAL_POWER_CODE) > 0
        ...
        # ----------------------------------------------------------------
        # Step 2b: Rank surviving companies by FFMC_CO, keep top 30
        # ----------------------------------------------------------------
        logger.info("Step 2b: Ranking post-exclusion universe by FFMC_CO, keeping top 30...")
        ...
        eligible_df = ranked_by_ffmc.head(30).copy()
        logger.info(f"Eligible companies after FFMC screen (top 30): {len(eligible_df)}")

        # ----------------------------------------------------------------
        # Step 3: Rank eligible 30 by ESG Risk Score (ascending)
        ...
        # ----------------------------------------------------------------
        # Step 4: Select top 20 by ESG Risk Score ranking
        # ----------------------------------------------------------------
        logger.info("Step 4: Selecting top 20 companies by ESG Risk Score...")

        top_20_df = scored_eligible_df.head(20).copy()
        logger.info(f"Selected {len(top_20_df)} companies for index")
        ...
        num_constituents = len(top_20_df)
        target_mcap_per_company = index_mcap / num_constituents

        top_20_df['Unrounded NOSH'] = (
            target_mcap_per_company
            / (top_20_df['Close Prc_EOD'] * top_20_df['FX/Index Ccy'])
        )
        top_20_df['Rounded NOSH'] = top_20_df['Unrounded NOSH'].round()

        # Free Float Factor and Capping Factor not applied per rulebook
        top_20_df['Free Float'] = 1
        top_20_df['Capping Factor'] = 1
        ...
        try:
            ...
            euepr_path = os.path.join(output_dir, f'EUEPR_df_{timestamp}.xlsx')

            logger.info(f"Saving EUEPR output to: {euepr_path}")
            with pd.ExcelWriter(euepr_path) as writer:
                EUEPR_df.to_excel(writer, sheet_name='Index Composition', index=False)
                inclusion_df.to_excel(writer, sheet_name='Inclusion', index=False)
                exclusion_df.to_excel(writer, sheet_name='Exclusion', index=False)
                top_20_df.to_excel(writer, sheet_name='Top 20 Selection', index=False)
                scored_eligible_df.to_excel(writer, sheet_name='ESG Ranked Eligible 30', index=False)
                eligible_df.to_excel(writer, sheet_name='FFMC Eligible 30', index=False)
            ...
            return {
                "status": "success",
                "message": "EUEPR review completed successfully",
                "data": {"euepr_path": euepr_path}
            }
        ...
        logger.error(f"Error during EUEPR review calculation: {str(e)}")
        logger.error(traceback.format_exc())
        return {
            "status": "error",
            "message": f"Error during EUEPR review calculation: {str(e)}",
