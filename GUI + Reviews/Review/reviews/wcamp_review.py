...
def run_wcamp_review(date, co_date, effective_date, index="WCAMP", isin="FR0014005GI3", 
    ...
    try:
        ...
        # Create selection_df from stock_eod_df for WCAMP constituents
        logger.info("Creating selection_df from WCAMP constituents...")
        selection_df = stock_eod_df[stock_eod_df['Index'] == 'WCAMP'].copy()

        if selection_df.empty:
            raise ValueError("No constituents found with Index = 'WCAMP' in stock_eod_df")
        ...
        logger.info(f"Found {len(selection_df)} WCAMP constituents")
        ...
        try:
            ...
            wcamp_path = os.path.join(output_dir, f'WCAMP_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving WCAMP output to: {wcamp_path}")
            with pd.ExcelWriter(wcamp_path) as writer:
                WCAMP_df.to_excel(writer, sheet_name='Index Composition', index=False)
