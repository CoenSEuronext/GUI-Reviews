...
def run_lc1ep_review(date, co_date, effective_date, index="LC1EP", isin="FR0013522554", 
                    area="US", area2="EU", type="STOCK", universe="Eurozone 300", 
                    feed="Reuters", currency="EUR", year=None):
    """
    Run the Low Carbon 100 Eurozone PAB index review calculation
    ...
    Args:
        ...
        index (str, optional): Index name. Defaults to "LC1EP".
        isin (str, optional): ISIN code. Defaults to "FR0013522554".
    ...
    try:
        ...
        ref_data = load_reference_data(
            ...
            sheet_names={
                'eu_taxonomy_pocket': 'Eurozone'  # Specify 'Eurozone' sheet for eu_taxonomy_pocket
            }
        )
        ...
        # Index Universe definition - Filter for EZ300 as mentioned in step 1 of 2.2 Review Selection
        logger.info("Filtering universe for EZ300 companies...")
        universe_df = universe_df[universe_df['index'].str.contains('EZ300', na=False)].copy()
        logger.info(f"Universe size after EZ300 filter: {len(universe_df)}")
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(output_dir, f'LC1EP_review_{timestamp}.xlsx')
