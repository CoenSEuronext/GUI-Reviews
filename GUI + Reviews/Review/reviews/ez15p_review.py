...
def run_ez15p_review(date, co_date, effective_date, index="EZ15P", isin="NL0012949101", 
    ...
    try:
        ...
        # Create top_150_df by merging back with full selection_df to preserve all columns including exclusion info
        top_150_df = selection_df[selection_df['ISIN code'].isin(top_150_eligible['ISIN code'])].copy()
        
        # Calculate the target market cap per company (equal weighting)
        target_mcap_per_company = index_mcap / top_n
        top_150_df['Unrounded NOSH'] = target_mcap_per_company / top_150_df['Close Prc_EOD']
        top_150_df['Rounded NOSH'] = top_150_df['Unrounded NOSH'].round()
        top_150_df['Free Float'] = 1
        top_150_df['Effective Date of Review'] = effective_date
        top_150_df['Currency'] = currency
        selection_df['Unrounded NOSH'] = target_mcap_per_company / selection_df['Close Prc_EOD']
        
        # Prepare EZ15P DataFrame
        EZ15P_df = (
            top_150_df[
        ...
        try:
            ...
            ez15p_path = os.path.join(output_dir, f'EZ15P_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving EZ15P output to: {ez15p_path}")
            with pd.ExcelWriter(ez15p_path) as writer:
                EZ15P_df.to_excel(writer, sheet_name='Index Composition', index=False)
