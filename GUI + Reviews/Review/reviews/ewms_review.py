...
def run_ewms_review(date, effective_date, co_date, index="EWMS", isin="NLIX00008564", 
    ...
    """
    Run the EWMS index review calculation
    ...
    Args:
        ...
        index (str, optional): Index name. Defaults to "EWMS"
    ...
    try:
        logger.info("Starting EWMS review calculation")
        ...
        # Flag existing companies in the EWMS index
        # Look up ISIN codes from stock_eod_df where the Index column matches 'EWMS'
        # Note: stock_eod_df uses 'Isin Code' (capital I, lowercase sin)
        ...
        universe_df['EWMS_selection'] = 0
        
        existing_mask = universe_df['Is_Existing'] == 1
        new_mask = universe_df['Is_Existing'] == 0
        
        universe_df.loc[existing_mask & (universe_df['Country_Cumulative_Percentage'] >= 68), 'EWMS_selection'] = 1
        universe_df.loc[new_mask & (universe_df['Country_Cumulative_Percentage'] >= 72), 'EWMS_selection'] = 1
        
        # Print summary statistics per country
        logger.info(f"\nEWMS Selection Summary by Country:")
        for country in universe_df['Country'].dropna().unique():
            country_df = universe_df[universe_df['Country'] == country]
            selected_count = country_df['EWMS_selection'].sum()
            existing_selected = country_df[existing_mask & (country_df['EWMS_selection'] == 1)].shape[0]
            new_selected = country_df[new_mask & (country_df['EWMS_selection'] == 1)].shape[0]
            logger.info(f"{country}: {selected_count} total ({existing_selected} existing, {new_selected} new)")
        
        # Print overall summary statistics
        logger.info(f"\nOverall EWMS Selection Summary:")
        logger.info(f"Total companies selected: {universe_df['EWMS_selection'].sum()}")
        logger.info(f"  - Existing companies selected: {universe_df[existing_mask & (universe_df['EWMS_selection'] == 1)].shape[0]}")
        logger.info(f"  - New companies selected: {universe_df[new_mask & (universe_df['EWMS_selection'] == 1)].shape[0]}")
        logger.info(f"Total FFMC covered: {universe_df[universe_df['EWMS_selection'] == 1]['Mcap in EUR'].sum() / universe_df['Mcap in EUR'].sum() * 100:.2f}%")
        ...
        selected_df = universe_df[universe_df['EWMS_selection'] == 1].copy()
        ...
        for col in capping_columns:
            universe_df.loc[universe_df['EWMS_selection'] == 1, col] = capped_df[col].values
        
        # For non-selected companies, set default values
        universe_df.loc[universe_df['EWMS_selection'] == 0, 'Capping Factor'] = 1.0
        universe_df.loc[universe_df['EWMS_selection'] == 0, 'Is Capped'] = False
        ...
        composition_df = universe_df[universe_df['EWMS_selection'] == 1][selected_columns].copy()
        composition_df = composition_df.sort_values('Company', ascending=True)

        # Analyze inclusions and exclusions
        selection_df = universe_df[universe_df['EWMS_selection'] == 1].copy()
        ...
        try:
            ...
            ewms_path = os.path.join(output_dir, f'EWMS_Review_{timestamp}.xlsx')
            
            logger.info(f"Saving output to: {ewms_path}")
            
            # Create the Excel file with all sheets
            with pd.ExcelWriter(ewms_path) as writer:
                # Write composition sheet
                composition_df.to_excel(writer, sheet_name='EWMS Composition', index=False)
                ...
                inclusions_df_sorted.to_excel(writer, sheet_name='EWMS Inclusions', index=False)

                exclusions_df_sorted = exclusions_df.sort_values('Company', ascending=True) if not exclusions_df.empty else exclusions_df
                exclusions_df_sorted.to_excel(writer, sheet_name='EWMS Exclusions', index=False)

                # Write full universe sheet
                universe_df.to_excel(writer, sheet_name='Full Universe', index=False)
            
            # Verify file was saved
            if os.path.exists(ewms_path):
                logger.info(f"File successfully saved to: {ewms_path}")
                ...
                return {
                    ...
                    "data": {
                        "ewms_path": ewms_path,
                        "summary": {
                            "total_companies": int(universe_df['EWMS_selection'].sum()),
                            "existing_companies_selected": int(universe_df[existing_mask & (universe_df['EWMS_selection'] == 1)].shape[0]),
                            "new_companies_selected": int(universe_df[new_mask & (universe_df['EWMS_selection'] == 1)].shape[0]),
                            "total_ffmc_coverage": float(universe_df[universe_df['EWMS_selection'] == 1]['Mcap in EUR'].sum() / universe_df['Mcap in EUR'].sum() * 100),
