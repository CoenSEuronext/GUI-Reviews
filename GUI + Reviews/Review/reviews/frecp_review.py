...
def run_frecp_review(date, co_date, effective_date, index="FRECP", isin="FR0013349057", 
    ...
    try:
        ...
        selection_df["Free Float"] = selection_df["Free Float Round:"].fillna(selection_df["Free Float"])
        ...
        # Sort by FFMC and select the top N lowest companies
        top_n = 20
        logger.info(f"Selecting top {top_n} companies with lowest FFMC...")
        bottom_20_df = selection_df.nsmallest(top_n, 'FFMC').copy()
        # Calculate the target market cap per company (equal weighting)
        target_mcap_per_company = index_mcap / top_n
        bottom_20_df['Unrounded NOSH'] = target_mcap_per_company / bottom_20_df['Close Prc_EOD']
        bottom_20_df['Rounded NOSH'] = bottom_20_df['Unrounded NOSH'].round()
        bottom_20_df['Capping Factor'] = 1
        bottom_20_df['Free Float'] = 1
        bottom_20_df['Effective Date of Review'] = effective_date
        bottom_20_df['Currency'] = currency
        selection_df['Unrounded NOSH'] = target_mcap_per_company / selection_df['Close Prc_EOD']
        # Prepare FRECP DataFrame
        FRECP_df = (
            bottom_20_df[
        ...
        try:
            ...
            frecp_path = os.path.join(output_dir, f'FRECP_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving FRECP output to: {frecp_path}")
            with pd.ExcelWriter(frecp_path) as writer:
                FRECP_df.to_excel(writer, sheet_name='Index Composition', index=False)
