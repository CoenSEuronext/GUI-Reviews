...
def run_emls_review(date, co_date, effective_date, index="EMLS", isin="NLIX00008903", 
    ...
    try:
        ...
        emls_universe = [
            ...
            {'Company': 'RHEINMETALL', 'ISIN': 'DE0007030009', 'MIC': 'XETR', 'Currency': 'EUR'},
            {'Company': 'PROSUS', 'ISIN': 'NL0013654783', 'MIC': 'XAMS', 'Currency': 'EUR'},
            ...
            {'Company': 'SAFRAN', 'ISIN': 'FR0000073272', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'LVMH', 'ISIN': 'FR0000121014', 'MIC': 'XPAR', 'Currency': 'EUR'},
            {'Company': 'AMAZON', 'ISIN': 'US0231351067', 'MIC': 'XNGS', 'Currency': 'USD'},
            {'Company': 'PALANTIR TECH', 'ISIN': 'US69608A1088', 'MIC': 'XNGS', 'Currency': 'USD'},
            {'Company': 'ORACLE CORP', 'ISIN': 'US68389X1054', 'MIC': 'XNYS', 'Currency': 'USD'},
            {'Company': 'NETFLIX INC.', 'ISIN': 'US64110L1061', 'MIC': 'XNGS', 'Currency': 'USD'},
            {'Company': 'NVIDIA CORP', 'ISIN': 'US67066G1040', 'MIC': 'XNGS', 'Currency': 'USD'},
            {'Company': 'TESLA INC', 'ISIN': 'US88160R1014', 'MIC': 'XNGS', 'Currency': 'USD'},
            {'Company': 'APPLE', 'ISIN': 'US0378331005', 'MIC': 'XNGS', 'Currency': 'USD'},
            {'Company': 'BROADCOM INC.', 'ISIN': 'US11135F1012', 'MIC': 'XNGS', 'Currency': 'USD'}
        ]

        # Convert to DataFrame when needed
        emls_df = pd.DataFrame(emls_universe)
        ff_df = ref_data['ff']

        
        # Add the required columns to the combined dataframe
        emls_df['Capping Factor'] = 1
        emls_df['Effective Date of Review'] = effective_date
        ...
        top_n =20  # 15 companies in EMLS universe
        ...
        try:
            ...
            emls_path = os.path.join(output_dir, f'EMLS_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving EMLS output to: {emls_path}")
            with pd.ExcelWriter(emls_path) as writer:
                EMLS_df.to_excel(writer, sheet_name='Index Composition', index=False)
