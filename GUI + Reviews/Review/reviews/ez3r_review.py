...
def run_ezgp_review(date, co_date, effective_date, index="EZGP", isin="FRESG0003540", 
    ...
    try:
        ...
        # Step 2.2: Carbon Emission Data filter - SKIPPED FOR NOW
        logger.info("Step 2.2: Carbon Emission Data filter - skipped for now")
        ...
        exclusion_gmo = False
        if col_181268702099:
            exclusion_gmo |= safe_numeric(selection_df[col_181268702099]) < 100
        if col_171016181199:
            exclusion_gmo |= safe_numeric(selection_df[col_171016181199]) > 0
        selection_df['exclusion_gmo'] = exclusion_gmo
        
        # Step 2.5j: Pesticides
        # Category: 172310122999 - Flagged 'PE1', 'PE2', 'PE3', or 'PE4'
        col_172310122999 = find_column_by_code(selection_df, '172310122999')
        if col_172310122999:
            selection_df['exclusion_pesticides'] = selection_df[col_172310122999].isin(['PE1', 'PE2', 'PE3', 'PE4'])
        else:
        ...
        for code in environmental_codes:
            ...
            if col:
                exclusion_environmental |= selection_df[col].isin([4, 5, '4', '5'])
