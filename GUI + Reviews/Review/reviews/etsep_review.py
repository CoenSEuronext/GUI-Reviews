...
def run_etsep_review(date, co_date, effective_date, index="ETSEP", isin="NLIX00007095", 
    ...
    try:
        ...
        # ETSEP universe
        etsep_universe = [
            {'Company': 'UNITED HEALTH GROUP', 'ISIN': 'US91324P1021', 'MIC': 'XNYS', 'Currency': 'USD'},
        ...
        etsep_df = pd.DataFrame(etsep_universe)
        ff_df = ref_data['ff']

        
        # Add the required columns to the combined dataframe
        etsep_df['Capping Factor'] = 1
        etsep_df['Effective Date of Review'] = effective_date
        ...
        top_n = 10  # 10 companies in ETSEP universe
        ...
        try:
            ...
            etsep_path = os.path.join(output_dir, f'ETSEP_df_{timestamp}.xlsx')
           
            # Save output with multiple sheets
            logger.info(f"Saving ETSEP output to: {etsep_path}")
            with pd.ExcelWriter(etsep_path) as writer:
                ETSEP_df.to_excel(writer, sheet_name='Index Composition', index=False)
