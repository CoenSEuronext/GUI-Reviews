...
try:
    ...
    # Create lookup dictionaries for ISINs from index_eod_df
    isin_lookup = {}
    gross_isin_lookup = {}
    net_isin_lookup = {}
    
    # Build the price ISIN lookup (Mnemo -> IsinCode)
    for idx, row in index_eod_df.iterrows():
        if not pd.isna(row['Mnemo']) and not pd.isna(row['IsinCode']):
            isin_lookup[row['Mnemo']] = row['IsinCode']
    
    # Build the gross and net ISIN lookups (ISIN Underlying Price Index -> ISIN Gross/Net Return version)
    for idx, row in index_eod_df.iterrows():
        if not pd.isna(row['ISIN Underlying Price Index']) and not pd.isna(row['ISIN Gross Return version']):
            gross_isin_lookup[row['ISIN Underlying Price Index']] = row['ISIN Gross Return version']
        
        if not pd.isna(row['ISIN Underlying Price Index']) and not pd.isna(row['ISIN Net Return version']):
            net_isin_lookup[row['ISIN Underlying Price Index']] = row['ISIN Net Return version']
    ...
    for mnemo in mnemonics:
        ...
        # Look up the ISIN from the isin_lookup dictionary
        ...
        results.append({
            ...
            'Price_Isin': price_isin  # Add the new Price_Isin column
        })
    ...
    try:
        ...
        # Create a lookup dictionary for ISINs from index_eod_df
        isin_lookup = {}
        for idx, row in index_eod_df.iterrows():
            if not pd.isna(row['Mnemo']) and not pd.isna(row['IsinCode']):
                isin_lookup[row['Mnemo']] = row['IsinCode']
        ...
        for mnemo in mnemonics:
            ...
            # Look up the Gross and Net ISINs based on the Price ISIN
