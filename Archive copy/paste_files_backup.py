...
def merge_csv_files(file1_path, file2_path, output_path):
    """Merge two CSV files, keeping the header from the first file only"""
    try:
        ...
        with open(file2_path, 'r', encoding='latin1', newline='') as f2:
            ...
            data2 = list(reader2)[1:]  # Skip header
        
        # Combine data
        merged_data = data1 + data2
        
        # Write the merged data
        with open(output_path, 'w', encoding='latin1', newline='') as out_file:
...
def get_merge_groups(date_str):
    ...
    return {
        "MANUAL": {
            ...
            "output_files": [
                f"EU_MANUAL_US_NXTD_STOCK_MERGED_{date_str}.csv",
                f"EU_MANUAL_US_NXTD_INDEX_MERGED_{date_str}.csv"
            ],
            "merge_pairs": [
        ...
        "EOD": {
            ...
            "output_files": [
                f"EU_EOD_US_EOD_STOCK_MERGED_{date_str}.csv",
                f"EU_EOD_US_EOD_INDEX_MERGED_{date_str}.csv"
            ],
            "merge_pairs": [
                (0, 1, 0),
                (2, 3, 1)
        ...
        "SOD": {
            ...
            "output_files": [
                f"EU_SOD_US_SOD_STOCK_MERGED_{date_str}.csv",
                f"EU_SOD_US_SOD_INDEX_MERGED_{date_str}.csv"
            ],
            "merge_pairs": [
                (0, 1, 0),
                (2, 3, 1)
